...
The header is now an array of text-strings. We need to extracts some float values. 
>>> value = '1e-7'
>>> value
'1e-7'
>>> float(value)
...
For rigolfr:
>>> ampl = np.loadtxt('../Rigol_DS1054/20180502_1.csv', delimiter=',', skiprows=2, usecols=0)
>>> ampl
array([ 0.   ,  0.   ,  0.   , ..., -0.028, -0.092, -0.092])

To add the timeline. We need to recalculate it from the information in the header. 
>>> ampl.size
1200
#   tstamp=np.array([(self.start + (x * self.samplerate)) for x in range(ampl.size)]) 
...
array([[-9.600e-06,  0.000e+00],
       ...
       [ 1.102e-04, -9.200e-02],
       [ 1.103e-04, -9.200e-02]])
This way the data array has the same form as data arrays imported using the other filereaders.
'''
